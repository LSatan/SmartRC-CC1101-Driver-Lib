...
int Sendpin = 6;  // int for Receive pin.
int Recivpin = 0; // int for Receive pin.
...
//ELECHOUSE_cc1101.setChsp(50);     // set Channle spacing (default = 50khz) you can set 25,50,80,100,150,200,250,300,350,405.
  ...
  NewRemoteReceiver::init(Recivpin, Interr, ReTrans);
...
void loop() {
 ...
 void ReTrans(unsigned int period, unsigned long address, unsigned long groupBit, unsigned long unit, unsigned long switchType, unsigned long dimLevelPresent, unsigned long dimLevel) {  // Disable the receiver; otherwise it might pick up the retransmit as well.
   ...
   if (debug == 1) {Serial.print("Wait... ");}
    ...
    NewRemoteTransmitter transmitter(address, Sendpin, period, Anz);
